...
new p5((p: p5) => {
  p.setup = () => {
    ...
    (p as any).strokeCap((p as any).SQUARE);
    ...
    GRIDCOUNT = p.ceil(p.sqrt(PRECODED.length));

    jumpx = (p.width - padding * 2) / (GRIDCOUNT - 1);
    jumpy = (p.height - padding * 2) / (GRIDCOUNT - 1);
  };
  ...
  p.draw = () => {
    ...
    for (let y = padding; y <= p.height - padding; y += jumpy) {
      for (let x = padding; x <= p.width - padding; x += jumpx) {
        char = "";
        p.stroke(colors[count % colors.length]);
  ...
  const drawShape = (x: any, y: any, radius: any, shape: any, hasDot: any, rotation: any) => {
    let angles: any[] = [];
    if (shape === "L") {
    ...
    for (let i = 0; i < angles.length; i++) {
      if (angles[i].length) {
        let sx = 0 + p.cos(angles[i][0]) * radius;
        let sy = 0 + p.sin(angles[i][1]) * radius;
        p.vertex(sx, sy);
      }
      const a = angles[i];
      let sx = 0 + p.cos(a) * radius;
      let sy = 0 + p.sin(a) * radius;
      p.vertex(sx, sy);
    }
    p.endShape();
    if (hasDot) {
      p.strokeWeight(STROKE_WEIGHT * 1.5);
      if (shape === "V") {
        p.point(0, (p.sin(-45) * radius) / 2);
      } else {
        p.point(0, 0);
      }
      p.strokeWeight(STROKE_WEIGHT);
